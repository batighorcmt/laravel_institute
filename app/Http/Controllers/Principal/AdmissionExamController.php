...
class AdmissionExamController extends Controller
{
    ...
    public function marks(School $school, AdmissionExam $exam)
    {
        ...
        $apps = \App\Models\AdmissionApplication::where('school_id', $school->id)
            ...
            ->when($exam->class_name, function ($q) use ($exam) {
                $q->where('class_name', $exam->class_name);
            })
        ...
        if ($exam->type === 'subject') {
            ...
            foreach ($submitted as $appId => $subArray) {
                foreach ($subArray as $subjectId => $val) {
                    ...
                    if ($val > (int) $subject->full_mark) {
                        $val = (int) $subject->full_mark;
                    }
        ...
        if ($exam->type === 'subject') {
            ...
            foreach ($acceptedApps as $appId) {
                ...
                foreach ($subjects as $sub) {
                    ...
                    $total += $obt;
                    if ($sub->pass_mark !== null && $obt < $sub->pass_mark) {
                        $failed++;
                    }
                }
    ...
    public function sendResultsSms(Request $request, School $school, AdmissionExam $exam)
    {
        ...
        $sortedResults = $results->sort(function ($a, $b) {
            $diff = $b->total_obtained - $a->total_obtained;
            if ($diff !== 0) {
                return $diff;
            }
        ...
        foreach ($sortedResults as $r) {
            $r->merit_position = $pos++;
        }

        // Optional limit for dry run (caps recipients)
        $limit = (int) $request->input('limit', 0);
        if ($limit > 0) {
            $sortedResults = $sortedResults->take(min(100, $limit));
        }
        ...
        if (empty($payloads)) {
            return back()->with('error', 'কোনো বৈধ মোবাইল নম্বর পাওয়া যায়নি।');
        }
        ...
        $chunkCount = \App\Services\SmsDispatch::dispatchChunks($school->id, auth()->id(), $payloads);

        if (\App\Services\SmsDispatch::shouldSendImmediately()) {
            return back()->with('success', 'মোট '.count($payloads).' প্রাপককে SMS পাঠানো হয়েছে ('.$chunkCount.' ব্যাচ)। লগ পৃষ্ঠায় বিস্তারিত দেখুন।');
        }

        return back()->with('success', 'মোট '.count($payloads).' প্রাপককে SMS পাঠানোর কাজ কিউ হয়েছে। '.$chunkCount.'টি ব্যাচে পাঠানো হবে। কিছুক্ষণ পর লগ চেক করুন।');
    }
}
