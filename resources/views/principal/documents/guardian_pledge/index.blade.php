...
@section('title', 'Documents: অভিভাবক অঙ্গীকার নামা')
...
<div class="container-fluid">
    <div class="card shadow-sm border-0" style="border-radius: 12px;">
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center" style="border-top-left-radius: 12px; border-top-right-radius: 12px;">
            <h5 class="mb-0"><i class="fas fa-file-signature mr-2"></i>অভিভাবক অঙ্গীকার নামা জেনারেটর</h5>
        </div>
        <div class="card-body">
            <!-- Filter Section -->
            <div class="form-row">
                ...
                <div class="form-group col-md-3 d-flex align-items-end">
                    <button type="button" class="btn btn-outline-primary btn-block" id="btnFilter" style="height: calc(1.5em + .75rem + 2px);">
                        <i class="fas fa-sync-alt mr-1"></i> লোড করুন
                    </button>
                </div>
            </div>
            ...
            <div id="instructionAlert" class="alert alert-info border-0 shadow-sm mt-4 d-flex align-items-center" style="border-radius: 8px;">
                ...
                <div>
                    <strong>নির্দেশনা:</strong> অভিভাবক অঙ্গীকার নামা প্রিন্ট করার জন্য অনুগ্রহ করে শিক্ষাবর্ষ ও শ্রেণি নির্বাচন করে "লোড করুন" বাটনে ক্লিক করুন।
                </div>
            </div>
            ...
            <div id="studentListContainer" class="mt-4" style="display: none;">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <button type="button" class="btn btn-success px-4" id="btnBulkPrint" style="border-radius: 30px;">
                        <i class="fas fa-print mr-1"></i> নির্বাচিত সকলের অঙ্গীকার নামা একসাথে প্রিন্ট
                    </button>
                </div>
                    ...
                    <table class="table table-bordered table-hover text-center align-middle mb-0" style="border-radius: 8px; overflow: hidden;">
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th style="text-align: left; vertical-align: middle; padding-left: 20px;">অভিভাবকের নাম (ডাটাবেজ অনুযায়ী)</th>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadStudentsUrl = @json(route('principal.institute.documents.guardian_pledge.load-students', $school));
    const printUrlTemplate = @json(route('principal.institute.documents.guardian_pledge.print', $school));
                ...
                if (data.students && data.students.length > 0) {
                    ...
                    data.students.forEach(student => {
                        ...
                        tr.innerHTML = `
                            ...
                            <td style="text-align: left; vertical-align: middle; padding-left: 20px;">
                                <span class="text-secondary">${student.guardian_name || '<i class="text-danger">তথ্য পাওয়া যায়নি</i>'}</span>
                            </td>
                            <td style="vertical-align: middle;">
                    ...
                    // Bind single print buttons
                    document.querySelectorAll('.btn-print-single').forEach(btn => {
                        btn.addEventListener('click', function() {
                            const studentId = this.getAttribute('data-id');
                            const printUrl = `${printUrlTemplate}?academic_year_id=${yearId}&class_id=${classId}&student_ids=${studentId}`;
                    ...
                    instructionAlert.innerHTML = `
                        ...
                        <div>
                            এই শ্রেণির জন্য কোনো সক্রিয় শিক্ষার্থী পাওয়া যায়নি।
                        </div>
                    `;
                }
            })
    ...
    // Auto-trigger filter if year and class are already chosen
    ...
    btnBulkPrint.addEventListener('click', function() {
        ...
        const printUrl = `${printUrlTemplate}?academic_year_id=${yearId}&class_id=${classId}&student_ids=${selectedIds}`;
        window.open(printUrl, '_blank');
    });
