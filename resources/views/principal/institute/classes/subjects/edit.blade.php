...
<div class="d-flex justify-content-between mb-3">
  <h1 class="m-0">ম্যাপিং সম্পাদনা: {{ $mapping->subject ? ($mapping->subject->bangla_name ? $mapping->subject->bangla_name.' ('.$mapping->subject->name.')' : $mapping->subject->name) : '(বিষয় মুছে ফেলা হয়েছে)' }}</h1>
  ...
  <div class="card-body">
    <form method="post" action="{{ route('principal.institute.classes.subjects.update',[$school,$class,$mapping]) }}">@csrf @method('PATCH')
      <div class="form-group">
        <label>বিষয়</label>
        <input type="text" class="form-control" value="{{ $mapping->subject ? ($mapping->subject->bangla_name ? $mapping->subject->bangla_name.' ('.$mapping->subject->name.')' : $mapping->subject->name) : '(বিষয় মুছে ফেলা হয়েছে)' }}" disabled>
      </div>
      @if($class->usesGroups())
      <div class="form-group">
        <label>গ্রুপ</label>
