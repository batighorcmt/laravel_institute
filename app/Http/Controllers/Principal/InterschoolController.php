...
class InterschoolController extends Controller
{
    ...
    public function printAppendixKa(Request $request, $school)
    {
        ...
        $seasonEvent = InterschoolSeasonEvent::with(['event', 'subEvent', 'players.student.currentEnrollment.class', 'season'])->findOrFail($seasonEventId);
        ...
        foreach ($seasonEvent->players as $player) {
            if ($player->student) {
                // Determine precise age
                if ($player->student->date_of_birth) {
                    $dob = \Carbon\Carbon::parse($player->student->date_of_birth);
                    $diff = $dob->diff($ageDateCarbon);
                    $player->calculated_age = [
                        'years' => $diff->y,
                        'months' => $diff->m,
                        'days' => $diff->d,
                    ];
