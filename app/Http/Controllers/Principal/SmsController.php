...
class SmsController extends Controller
{
    public function panel(School $school)
    {
        ...
        $balanceData = Cache::remember($cacheKey, now()->addMinutes(3), function () use ($apiKey) {
            ...
            if (! $apiKey) {
                $out['error'] = 'API key configured নয়';
            ...
            try {
                ...
                if ($balance === null) {
                    if (preg_match('/([0-9]+(?:\.[0-9]+)?)/', (string) $body, $m)) {
                        $balance = $m[1];
                    }
                }
    ...
    public function send(Request $request, School $school)
    {
        ...
        if (! empty($data['recipients_json'])) {
            ...
            if (is_array($arr)) {
                foreach ($arr as $r) {
                    ...
                    if (strlen($num) === 13 && str_starts_with($num, '880')) {
                        $num = '0'.substr($num, 3);
                    }
                    if (! $num) {
                        continue;
        ...
        if (empty($recipients)) {
            ...
            if ($target === 'custom_numbers' && ! empty($data['numbers'])) {
                ...
                foreach ($parts as $n) {
                    ...
                    if ($n) {
                        $recipients[$n] = ['recipient_type' => 'custom', 'recipient_category' => 'custom_numbers'];
                    }
                }
            }
            ...
            if ($target === 'students_all' && ! empty($data['class_id'])) {
                ...
                if (! empty($data['section_id'])) {
                    $q->where('student_enrollments.section_id', (int) $data['section_id']);
                }
        ...
        $userId = auth()->id();
        $chunkCount = SmsDispatch::dispatchChunks($school->id, $userId, $payloads);
        $chunkSize = max(1, (int) config('sms.chunk_size', 50));

        if (SmsDispatch::shouldSendImmediately()) {
            return back()->with('success', 'মোট '.$recipients->count().' টি নম্বরে SMS পাঠানো হয়েছে ('.$chunkCount.' ব্যাচ)। লগ পৃষ্ঠায় বিস্তারিত দেখুন।');
        }

        return back()->with('success', 'মোট '.$recipients->count().' টি নম্বরে পাঠানোর জন্য '.$chunkCount.'টি ব্যাচে (প্রতি ব্যাচ ~'.$chunkSize.' বার্তা) কিউ করা হয়েছে। কিছুক্ষণ পর লগ চেক করুন।');
    }

    public function logs(Request $request, School $school)
    {
        $q = SmsLog::forSchool($school->id)->orderByDesc('created_at');
        if ($v = $request->get('number')) {
