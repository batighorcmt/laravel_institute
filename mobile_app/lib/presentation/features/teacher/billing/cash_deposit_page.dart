...
class _CashDepositPageState extends State<CashDepositPage> {
  ...
  Future<void> _fetchTransferStats() async {
    ...
    try {
      final res = await _dio.get(
        'billing/reports/teacher-cash-transfer',
        queryParameters: {
      ...
      if (mounted) {
        ...
        _breakdown = breakdown
            .where(
              (item) =>
                  (double.tryParse(
                        item['remaining_to_request']?.toString() ?? '0',
                      ) ??
                      0) >
                  0,
            )
  ...
  Future<void> _showConfirmation() async {
    if (_totalToDeposit <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('জমা দেওয়ার জন্য কোনো পরিমাণ নেই')),
      );
      return;
    }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'আপনি কি মোট ৳${_totalToDeposit.toStringAsFixed(2)} টাকা ক্যাশিয়ারের নিকট জমা দেওয়ার আবেদন করতে চান?',
        ),
        actions: [
          TextButton(
  ...
  Future<void> _submitDeposit() async {
    ...
    for (int i = 0; i < _breakdown.length; i++) {
      ...
      final maxAmt =
          double.tryParse(
            _breakdown[i]['remaining_to_request']?.toString() ?? '0',
          ) ??
          0;
    ...
    try {
      await _dio.post(
        'billing/reports/teacher-cash-transfer/deposit',
        data: {'items': items},
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('জমা আবেদন সফলভাবে ক্যাশিয়ারের নিকট পাঠানো হয়েছে!'),
          ...
          children: [
            ...
            Row(
              children: [
                _StatCard(
                  title: 'সংগৃহীত',
                  amount: _data?['total_collected']?.toString() ?? '0',
                  color: Colors.blue,
                ),
                const SizedBox(width: 12),
                _StatCard(
                  title: 'জমাকৃত (গৃহীত)',
                  amount: _data?['total_received']?.toString() ?? '0',
                  color: Colors.green,
                ),
              ],
            ),
            ...
            Row(
              children: [
                ...
                if ((double.tryParse(
                          _data?['total_pending']?.toString() ?? '0',
                        ) ??
                        0) >
                    0) ...[
                  const SizedBox(width: 12),
            ...
            if (_breakdown.isEmpty)
              ...
              ..._breakdown.asMap().entries.map(
                (entry) => _buildBreakdownRow(entry.key, entry.value),
              ),
            const SizedBox(height: 28),
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text(
                '৳${_totalToDeposit.toStringAsFixed(0)}',
                style: const TextStyle(
                  fontWeight: FontWeight.w900,
                  fontSize: 24,
                  color: Color(0xFF00BF6D),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: (_isDepositing || _totalToDeposit <= 0)
                  ? null
                  : _showConfirmation,
              style: ElevatedButton.styleFrom(
