...
<head>
    ...
    <style>
        body { font-family: 'Nikosh', sans-serif; font-size: 14px; margin: 0; padding: 10px; }
        ...
        .ka-form + .ka-form { page-break-before: always; break-before: page; margin-top: 20px; }
        @media print {
            @page { size: landscape; margin: 10mm; }
            .ka-form + .ka-form { page-break-before: always; break-before: page; }
        }
    </style>
</head>
<body onload="window.print()">
    ...
    @if($isGroupedSingleEvents)
        @forelse($groupedPlayers as $entry)
            <div class="ka-form">
                ...
                <div style="margin-top: 10px; margin-bottom: 10px;">
                    <div style="font-weight: bold; font-size: 15px; margin-bottom: 5px;">
                        ইভেন্টের নাম: {{ implode(', ', $entry['events']) ?: '—' }}
                    </div>
                ...
                @php
                    ...
                    $dobRaw = $student?->getRawOriginal('date_of_birth');
                    $admDateRaw = $student?->getRawOriginal('admission_date');
                @endphp
                ...
                <table>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>{{ $student?->father_name_bn ?? $student?->father_name_en }}</td>
                            <td>{{ $student?->mother_name_bn ?? $student?->mother_name_en }}</td>
                            <td>{{ $student?->currentEnrollment?->class?->bangla_name ?: ($student?->currentEnrollment?->class?->name ?? '-') }}</td>
                            <td>{{ en2bn($student?->currentEnrollment?->roll_no ?? '-') }}</td>
                            ...
                            <td>
                                @if($dobRaw && isset($entry['calculated_age']))
                                    {{ en2bn($entry['calculated_age']['years']) }} বছর,
                                    {{ en2bn($entry['calculated_age']['months']) }} মাস,
                                    {{ en2bn($entry['calculated_age']['days']) }} দিন
                                @else
                                    -
                                @endif
                            </td>
                            <td></td>
                            <td>{{ en2bn($entry['attendance_days'] ?? 0) }}</td>
                            <td></td>
                        </tr>
                    </tbody>
                </table>
        ...
        @empty
            <div class="ka-form">
                <h2 class="text-center" style="margin: 0; padding: 0;">পরিশিষ্ট "ক"</h2>
                <p class="text-center" style="margin-top: 20px;">কোনো একক ইভেন্টের খেলোয়াড় পাওয়া যায়নি</p>
            </div>
        @endforelse
    @else
        <div class="ka-form">
