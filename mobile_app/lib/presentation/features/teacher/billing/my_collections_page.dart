...
class _MyCollectionsPageState extends State<MyCollectionsPage> {
  ...
  Future<void> _fetchCollections({bool isLoadMore = false}) async {
    ...
    try {
      final res = await _dio.get('billing/reports/teacher-collections', queryParameters: {
      ...
      if (mounted) {
        ...
        final List<dynamic> newCollections = newData['collections'] as List? ?? [];
  ...
  Future<void> _downloadReceipt(dynamic paymentId, dynamic receiptNo) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('ডাউনলোড ব্যর্থ হয়েছে: $e'), backgroundColor: Colors.red),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('সংগ্রহের ইতিহাস', style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  Widget _buildBody() {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: _collections.length + 3, // Header, Summary, Title, Collections...
        itemBuilder: (context, index) {
          ...
          if (index == 1) {
            return Column(
              ...
              children: [
                ...
                if (summary.isEmpty) _buildEmptyState('No summary available') else _buildSummaryList(summary),
                const SizedBox(height: 24),
                _buildSectionTitle('Detailed Collections', Icons.receipt_long_outlined),
                const SizedBox(height: 12),
              ],
            );
          }
          ...
          if (_isLoadMoreLoading) {
            return const Padding(
              ...
              child: Center(child: CircularProgressIndicator(color: Color(0xFF00BF6D), strokeWidth: 2)),
            );
          }
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.calendar_today_rounded, color: Color(0xFF00BF6D), size: 18),
          ),
          const SizedBox(width: 14),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('তারিখ সীমা', style: TextStyle(color: Colors.grey.shade500, fontSize: 11, fontWeight: FontWeight.bold)),
              const SizedBox(height: 2),
              Text(
                '${DateFormat('dd MMM, yyyy').format(_fromDate)} - ${DateFormat('dd MMM, yyyy').format(_toDate)}',
                style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF1A1D1F)),
              ),
            ],
          ),
        ],
    ...
    return Row(
      children: [
        Container(
          width: 4,
          height: 18,
          decoration: BoxDecoration(color: const Color(0xFF00BF6D), borderRadius: BorderRadius.circular(2)),
        ),
        const SizedBox(width: 10),
        Text(
          title,
          style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: Color(0xFF1A1D1F), letterSpacing: 0.5),
        ),
      ],
    );
  }
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  Text(
                    '৳${s['paid']}',
                    style: const TextStyle(color: Color(0xFF00BF6D), fontWeight: FontWeight.w900, fontSize: 14),
                  ),
                  const SizedBox(height: 2),
                  Text(
                    'Due: ৳${s['due']}',
      ...
      child: ClipRRect(
        ...
        child: Column(
          children: [
            // Student Info Header
            InkWell(
              onTap: () => _downloadReceipt(c['id'], c['receipt_no'] ?? c['id']),
              child: Padding(
                  ...
                  children: [
                    Hero(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          shape: BoxShape.circle,
                          border: Border.all(color: const Color(0xFF00BF6D).withOpacity(0.2), width: 2),
                        ),
                        child: CircleAvatar(
                          radius: 26,
                          backgroundColor: const Color(0xFFF0FDF4),
                          backgroundImage: photoUrl.isNotEmpty ? CachedNetworkImageProvider(photoUrl) : null,
                          child: photoUrl.isEmpty ? const Icon(Icons.person, color: Color(0xFF00BF6D)) : null,
                        ),
                      ),
                    ),
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            ...
                            style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 15, color: Color(0xFF1A1D1F)),
                          ),
                          const SizedBox(height: 4),
                          Text(
                          ...
                          Text(
                            ...
                            style: TextStyle(fontSize: 11, color: const Color(0xFF00BF6D), fontWeight: FontWeight.bold),
                          ),
                        ],
                      ),
                    ),
                    const Icon(Icons.download_for_offline_outlined, color: Color(0xFF00BF6D), size: 24),
                  ],
                ),
              ),
            ),
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...items.map((item) {
                    final double fine = double.tryParse(item['fine']?.toString() ?? '0') ?? 0;
                    final double waiver = double.tryParse(item['waiver']?.toString() ?? '0') ?? 0;
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                if (item['month'] != null)
                                  Text(_formatBengaliMonth(item['month']?.toString()), style: TextStyle(fontSize: 11, color: Colors.grey.shade500)),
                              ],
                            ),
                          ),
                          Column(
                            ...
                            children: [
                              Text(
                                '৳${item['amount']}',
                                style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 13, color: Color(0xFF1A1D1F)),
                              ),
                              if (fine > 0 || waiver > 0)
                                Row(
                                  children: [
                                    if (fine > 0)
                                      Text('জরিমানা: ৳$fine ', style: const TextStyle(fontSize: 9, color: Colors.red, fontWeight: FontWeight.bold)),
                                    if (waiver > 0)
                                      Text('মওকুফ: ৳$waiver', style: const TextStyle(fontSize: 9, color: Color(0xFF00BF6D), fontWeight: FontWeight.bold)),
                                  ],
                                ),
                            ],
                          ),
              ...
              child: Row(
                ...
                children: [
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('রিসিট নম্বর', style: TextStyle(color: Colors.grey.shade500, fontSize: 10, fontWeight: FontWeight.bold)),
                      Text(
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.end,
                    children: [
                      Text('মোট গৃহিত অর্থ', style: TextStyle(color: Colors.grey.shade500, fontSize: 10, fontWeight: FontWeight.bold)),
                      Text(
                        '৳${c['amount_paid']}',
                        style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 16, color: Color(0xFF00BF6D)),
                      ),
                    ],
                  ),
                ],
