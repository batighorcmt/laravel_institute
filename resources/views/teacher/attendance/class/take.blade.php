...
@section('title','ছাত্র-ছাত্রী হাজিরা - ' . $schoolClass->name . ' ' . $section->name)
@section('content')
<div class="d-flex justify-content-between mb-3">
  <h1 class="m-0">উপস্থিতি নিন - {{ $schoolClass->name }} {{ $section->name }}</h1>
  <a href="{{ route('teacher.institute.attendance.class.index', $school) }}" class="btn btn-secondary">
...
@if($enrollments->count() > 0)
    ...
    <form method="POST" action="{{ route('teacher.institute.attendance.class.store', $school) }}" id="attendanceForm">
        @csrf
