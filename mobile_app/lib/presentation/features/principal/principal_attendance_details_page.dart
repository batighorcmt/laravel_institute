  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
          _DailyClassAttendanceTab(initialDate: widget.date),
          const _MonthlyClassAttendanceTab(),
        ],
      ),
    );
  }
...
class _DailyClassAttendanceTab extends StatefulWidget {
  final String? initialDate;
  const _DailyClassAttendanceTab({this.initialDate});

  @override
  State<_DailyClassAttendanceTab> createState() =>
      _DailyClassAttendanceTabState();
}

class _DailyClassAttendanceTabState extends State<_DailyClassAttendanceTab> {
  late final Dio _dio;
            ...
            builder: (context, snapshot) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, idx) {
                  ...
                  final classNameBn = cls['class_name_bn']?.toString();
                  final className =
                      (classNameBn != null && classNameBn.isNotEmpty)
                      ? classNameBn
                      : (cls['class_name'] ??
                            cls['name'] ??
                            cls['className'] ??
                  ...
                  for (final s in sections) {
                    final sNameBn = s['section_name_bn']?.toString();
                    final sName = (sNameBn != null && sNameBn.isNotEmpty)
                        ? sNameBn
...
class _MonthlyClassAttendanceTab extends StatefulWidget {
  const _MonthlyClassAttendanceTab();

  @override
  State<_MonthlyClassAttendanceTab> createState() =>
      _MonthlyClassAttendanceTabState();
}

class _MonthlyClassAttendanceTabState
    extends State<_MonthlyClassAttendanceTab> {
  late final Dio _dio;
  late Future<Map<String, dynamic>> _future;
  ...
  Future<Map<String, dynamic>> _fetch() async {
    final resp = await _dio.get(
      'principal/reports/attendance-monthly',
        ...
        Expanded(
          child: FutureBuilder<Map<String, dynamic>>(
            ...
            builder: (context, snapshot) {
              ...
              final classWise = List<dynamic>.from(
                (data['class_wise'] as List?) ?? [],
              );
              ...
              return ListView(
                children: [
                  ...
                  if (classWise.isEmpty)
                    ...
                    ...classWise.map((rawCls) {
                      final cls = Map<String, dynamic>.from(rawCls as Map);
                      final classNameBn = cls['class_name_bn']?.toString();
                      final className =
                          (classNameBn != null && classNameBn.isNotEmpty)
                          ? classNameBn
                          : (cls['class_name'] ?? '').toString();
                      final sections = List<dynamic>.from(
                      ...
                      return Card(
                        ...
                        child: ExpansionTile(
                          ...
                          subtitle: Text('${sections.length} টি শাখা'),
                          children: sections.map((rawSec) {
                            final s = Map<String, dynamic>.from(rawSec as Map);
                            final rank = toInt(s['rank']);
                            final sectionNameBn = s['section_name_bn']
                                ?.toString();
                            final sectionName =
                                (sectionNameBn != null &&
                                    sectionNameBn.isNotEmpty)
                                ? sectionNameBn
                                : (s['section_name'] ?? '').toString();
                            final total = toInt(s['total_students']);
                            final present = toInt(s['present_total']);
                            final daysTakenSec = toInt(
                              s['days_attendance_taken'],
                            );
                            final pct = toPct(s['percentage']);
                            return ListTile(
                              dense: true,
                              leading: RankBadge(rank: rank),
                              title: Text('শাখা: $sectionName'),
                              subtitle: Text(
                                'শিক্ষার্থী $total • হাজিরা নেওয়া হয়েছে $daysTakenSec দিন • উপস্থিতি $present',
