...
/// Staff self check-in/out. Mirrors
/// teacher/self_attendance_page.dart 1:1 but points at the staff/attendance*
/// endpoints and resolves school context via the 'staff' role only.
class StaffSelfAttendancePage extends ConsumerStatefulWidget {
  const StaffSelfAttendancePage({super.key});

  @override
  ConsumerState<StaffSelfAttendancePage> createState() =>
      _StaffSelfAttendancePageState();
}

class _StaffSelfAttendancePageState
    extends ConsumerState<StaffSelfAttendancePage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('হাজিরা (Self Attendance)')),
      body: Container(
  ...
  Future<bool> _submitCheckIn() async {
    ...
    try {
      final resp = await _dio.post('staff/attendance', data: form);
  ...
  Future<bool> _submitCheckout() async {
    ...
    final form = FormData.fromMap(data);
    try {
      final resp = await _dio.post('staff/attendance/checkout', data: form);
      dynamic payload = resp.data;
      Map<String, dynamic>? record;
      if (payload is Map) {
        if (payload['data'] is Map) {
  ...
  Future<void> _fetchSettings() async {
    try {
      final resp = await _dio.get('staff/attendance/settings');
      final data = resp.data;
  ...
  static const _queueKey = 'staff_attendance_offline_queue_v1';
    ...
    for (final item in list) {
      if (item is Map) {
        try {
          ...
          final schoolId = _schoolId ?? item['school_id'];
          final lat = item['lat'];
          final lng = item['lng'];
          ...
          await _dio.post(
            type == 'checkin' ? 'staff/attendance' : 'staff/attendance/checkout',
            data: form,
          );
        } catch (_) {
          remaining.add(item);
  ...
  void _handleDioError(DioException e) {
    ...
    if (e.response?.statusCode == 422 &&
      ...
      serverMsg = 'স্কুল কনটেক্সট পাওয়া যায়নি – দয়া করে আবার চেষ্টা করুন।';
    }
    ...
    try {
      ...
      if (profile != null) {
        for (final r in profile.roles) {
          if (r.role.toString().toLowerCase() == 'staff' && r.schoolId != null) {
            sid = r.schoolId;
            break;
          }
        }
...
class _StatusHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final status = _statusLabel(record!['status']?.toString());
    return Card(
      elevation: 0,
