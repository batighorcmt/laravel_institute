...
class AttendanceController extends Controller
{
    ...
    public function take(School $school, Request $request)
    {
        ...
        $enrollments = StudentEnrollment::with('student')
            ...
            ->when($yearVal, function ($q) use ($yearVal) {
                $q->where('academic_year_id', $yearVal);
            })
            ->orderBy('roll_no')
            ->get();
    ...
    public function store(School $school, Request $request)
    {
        ...
        try {
            ...
            $smsReport = $smsService->enqueueAttendanceSms(
                $school,
                $request->attendance,
                $classId,
                $sectionId,
            ...
            $message .= ' '.$smsReport['sent'].' SMS and Push notifications sent.';
        ...
        foreach ($attendanceData as $studentId => $data) {
            ...
            try {
                ...
                if ($ok) {
                    ...
                    $skipped[] = ['student_id' => $student->id, 'reason' => 'send_failed', 'response' => $resp];
                }
            } catch (\Throwable $e) {
    ...
    private function enqueueAttendanceSms(School $school, array $attendanceData, $classId, $sectionId, $date, $isExistingRecord, array $previousStatuses = [])
    {
        ...
        SmsDispatch::dispatchChunks($school->id, Auth::id(), $payloads);
