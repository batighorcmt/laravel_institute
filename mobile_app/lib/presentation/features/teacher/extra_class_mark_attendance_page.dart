...
class ExtraClassMarkAttendancePage extends StatefulWidget {
  ...
  final String title;
  const ExtraClassMarkAttendancePage({
    super.key,
    required this.extraClassId,
    required this.title,
  });

  @override
  State<ExtraClassMarkAttendancePage> createState() =>
      _ExtraClassMarkAttendancePageState();
}

class _ExtraClassMarkAttendancePageState
    extends State<ExtraClassMarkAttendancePage> {
  late final Dio _dio;
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final r = await _dio.get(
        'teacher/students-attendance/extra/classes/${widget.extraClassId}/students',
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final r = await _dio.post(
        'teacher/students-attendance/extra/classes/${widget.extraClassId}/attendance',
        data: body,
      );
      if (!mounted) return;
          ...
          : Column(
              children: [
                _HeaderBar(
                  ...
                  onSelectAllExcused: () =>
                      _selectAll(AttendanceStatus.excused),
                ),
        ...
        child: Row(
          children: [
            ...
            _StatusButton(
              ...
              tooltip: 'ছুটি',
              color: Colors.blue,
              selected: row.status == AttendanceStatus.excused,
